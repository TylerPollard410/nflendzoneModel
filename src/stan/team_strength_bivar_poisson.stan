// Joint home/away count model (Poisson-log) with offense/defense latents
functions {
...
parameters {
  ...
  // Global scoring intercept on log-scale
  real alpha_score;
}
...
model {
  ...
  // Scoring intercept: prior around typical NFL points per team (~21)
  alpha_score ~ normal(log(21), 0.5);
  
  // Likelihood (independent Poisson given latents)
  {
    ...
    home_score ~ poisson_log(eta[ : , 1]);
    away_score ~ poisson_log(eta[ : , 2]);
  }
}
generated quantities {
  ...
  // Per-game log-lik (sum of two Poisson-log)
  vector[N_games] log_lik;
  {
    ...
    for (g in 1 : N_games) 
      log_lik[g] = poisson_log_lpmf(home_score[g] | eta[g, 1])
                   + poisson_log_lpmf(away_score[g] | eta[g, 2]);
  }
  ...
  array[N_games] int sim_home_win;
  {
    matrix[N_games, 2] eta = compute_eta_home_away(home_team, away_team,
                               ...
                               N_games);
    for (g in 1 : N_games) {
      int y_h = poisson_log_rng(eta[g, 1]);
      int y_a = poisson_log_rng(eta[g, 2]);
