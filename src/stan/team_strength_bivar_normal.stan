// Joint home/away scores with offense/defense latents and bivariate Normal errors
functions {
  // Compute per-game expected home and away scores.
  // team_off_strength and team_def_strength are arrays (indexed by week) of vectors (indexed by team).
  matrix compute_mu_home_away(array[] int home_team, array[] int away_team,
    ...
    for (g in 1 : N_games) {
      ...
      int h = home_team[g];
      int a = away_team[g];
      mu[g, 1] = team_off_strength[w][h] - team_def_strength[w][a];
      if (hfa[g] == 1) 
        mu[g, 1] += team_hfa[s][h];
      mu[g, 2] = team_off_strength[w][a] - team_def_strength[w][h];
    }
    return mu;
  }
  
  // Map each week to the season index (take the max season index seen for that week).
  ...
  // Indicator for whether a week is the first week of a season (based on fw_season_idx per game).
  ...
  // Indicator for whether a week is the last week of a season (based on lw_season_idx per game).
...
parameters {
  ...
  // Bivariate Normal observation
  vector<lower=0>[2] sigma_score; // [sigma_home, sigma_away]
  cholesky_factor_corr[2] L_Omega; // correlation structure
  
  // Global points-scale intercept to anchor offense/defense
  real alpha_score_points;
}
...
model {
  ...
  sigma_score ~ student_t(3, 0, 10);
  L_Omega ~ lkj_corr_cholesky(3);
  
  // Points-scale intercept: typical team score ~ 21–24
  alpha_score_points ~ normal(23, 5);
  
  // Likelihood
  {
    matrix[N_games, 2] mu = compute_mu_home_away(home_team, away_team,
                              week_idx, season_idx, hfa, team_off_strength,
                              team_def_strength, team_hfa, N_games);
    
    matrix[2, 2] L_Sigma = diag_pre_multiply(sigma_score, L_Omega);
    ...
    for (g in 1 : N_games) {
      ...
      y_g[1] = home_score[g];
      y_g[2] = away_score[g];
      mu_g[1] = alpha_score_points + mu[g, 1];
      mu_g[2] = alpha_score_points + mu[g, 2];
      y_g ~ multi_normal_cholesky(mu_g, L_Sigma);
    }
  }
}
generated quantities {
  ...
  vector[N_games] log_lik;
  {
    ...
    for (g in 1 : N_games) {
      ...
      mu_g[1] = alpha_score_points + mu[g, 1];
      mu_g[2] = alpha_score_points + mu[g, 2];
      log_lik[g] = multi_normal_cholesky_lpdf(y_g | mu_g, L_Sigma);
    }
  }
}
