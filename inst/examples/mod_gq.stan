...
transformed parameters {
  ...
  team_strength[1] = to_vector(z_team_strength_init)
...
generated quantities {
  ...
  
  {
    ...
    for (fw in 1 : N_future_weeks) {
      ...
      if (is_first == 1) {
        ts_cur = phi_season_team_strength_innovation * ts_cur
        ...
        if (season_target <= N_seasons) {
                           ...
                           + normal_rng(0, 1) * sigma_league_hfa_innovation;
          {
            ...
            for (t in 1 : (N_teams - 1)) 
              zh_raw[t] = normal_rng(0, 1);
            vector[N_teams] zh0 = sum_to_zero_constrain(zh_raw);
            team_hfa_cur = rep_vector(league_hfa_cur, N_teams)
                           + zh0 * sigma_team_hfa;
          }
        }
        season_cur = season_target;
      } else {
        ts_cur = phi_weekly_team_strength_innovation * ts_cur
  ...
  for (k in 1 : N_oos) {
    ...
    if (w <= N_weeks) {
      ...
      if (is_last_week[last_w] == 1) {
        ts_target = phi_season_team_strength_innovation
                    * filtered_team_strength;
      } else {
        ts_target = phi_weekly_team_strength_innovation
                    * filtered_team_strength;
      }
