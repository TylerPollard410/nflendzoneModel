...
model {
  ...
  // Team HFA
  for (s in 1 : N_seasons) 
    z_team_hfa_deviation[s] ~ std_normal(); //normal(0, sum_to_zero_scale);
  ...
  for (w in 1 : (N_weeks - 1)) 
    z_weekly_team_strength_innovation[w] ~ std_normal(); //normal(0, sum_to_zero_scale);
  ...
  for (s in 1 : (N_seasons - 1)) 
    z_season_team_strength_innovation[s] ~ std_normal(); //normal(0, sum_to_zero_scale);
